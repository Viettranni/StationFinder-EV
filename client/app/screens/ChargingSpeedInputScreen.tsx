...
const ChargingSpeedInputScreen: React.FC = observer(() => {
  ...
  const currentValue = vm.selectedChargingSpeed ?? 0;
  const displayValue = currentValue.toString();

  const handleNumberPress = (num: string) => {
    const newValue = currentValue === 0 ? num : `${currentValue}${num}`;
    vm.selectChargingSpeed(Number(newValue));
  };

  const handleBackspace = () => {
    const str = currentValue.toString();
    if (str.length > 1) {
      vm.selectChargingSpeed(Number(str.slice(0, -1)));
    } else {
      vm.selectChargingSpeed(0);
    }
  };

  const handleContinue = () => {
    if (!vm.selectedChargingSpeed || vm.selectedChargingSpeed <= 0) {
      alert("Please enter a valid charging speed before continuing.");
      return;
    }
  ...
  const isDisabled = !vm.selectedChargingSpeed || vm.selectedChargingSpeed <= 0;
      ...
      <View style={styles.content}>
        ...
        <View style={styles.titleContainer}>
          <Text style={[styles.title, { color: isDark ? "#fff" : "#000" }]}>
            What's your charging speed?
          </Text>
          <Text style={[styles.subtitle, { color: isDark ? "#aaa" : "#555" }]}>
            Enter the maximum charging power your vehicle supports.
          </Text>
        </View>
        ...
        <View style={styles.inputContainer}>
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              vm.selectChargingSpeed(isNaN(num) ? 0 : num);
            }}
            placeholder="0"
...
export default ChargingSpeedInputScreen;
